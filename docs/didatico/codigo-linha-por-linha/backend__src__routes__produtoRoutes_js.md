...
| 2 | `const produtoController = require('../controllers/produtoController');` | Declara uma constante, ou seja, uma variável que não deve ser reatribuída depois. |
...
| 8 | `router.get('/produtos', autenticar, permitirPerfis('admin', 'viewer'), produtoController.listar);` | Cria uma rota GET da API, usada para consultar/listar dados. |
| 9 | `router.post('/produtos', autenticar, permitirPerfis('admin'), produtoController.criar);` | Cria uma rota POST da API, usada para cadastrar/enviar dados. |
| 10 | `router.put('/produtos/:id', autenticar, permitirPerfis('admin'), produtoController.atualizar);` | Cria uma rota PUT da API, usada para atualizar dados. |
| 11 | `router.delete('/produtos/:id', autenticar, permitirPerfis('admin'), produtoController.remover);` | Cria uma rota DELETE da API, usada para excluir dados. |
